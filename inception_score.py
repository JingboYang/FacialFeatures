...
if __name__=='__main__':
    ...
    #filenames = glob.glob(os.path.join('./testPics', '*.*'))
    '''

    images = []
    filenames = glob.glob('../output/perceptron_11_19__01_22_39/*_gen.png')
    print(filenames)
    images.extend([get_images(filename) for filename in filenames])
    print(get_inception_score(images))
    ...
    for i in range(40):
        ...
        images = []
        images.extend([get_images(filename) for filename in filenames])
        score_mean, score_variance = (get_inception_score(images)) 
        means.append(score_mean)
        variances.append(score_variance)
        print(score_mean, score_variance)
    
        pickle.dump( (means, variances), open( "save_2.p", "wb" ) )